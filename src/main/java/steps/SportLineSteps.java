...
public class SportLineSteps {
    ...
    public void buscarYValidadElementosEnSportLine(){
        for (int i = 0; i < listaProductos.size(); i++){
            GoogleChromeDriver.driver.findElement(sportLinePage.getTxtBuscador()).sendKeys(
                    listaProductos.get(i).get("NombreProducto"));
            ...
            Assert.assertEquals(listaProductos.get(i).get("NombreProducto").replaceAll("\\s+",""),
                    GoogleChromeDriver.driver.findElement(sportLinePage.getTxtElementoBusqueda()).getText().toString()
                            .replaceAll("\\s+",""));
        }
    }
